...
root = root = '/Users/edf/repos/cec_ng/pathways/fig/sf_electrification/elec/'
...
vlim_hour_peak = (np.round(iaq_high.hourly_peak_loads.min().min() / 1000), 
                  np.round(fhe_high.hourly_peak_loads.max().max() / 1000))

vlim_month_lf = (np.round(iaq_high.monthly_load_factors.min().min(), 2),
                 np.round(fhe_high.monthly_load_factors.max().max(), 2))
...
plot.CompareInputProfiles(profiles, names, units, root = root, figsize=(20,25))
